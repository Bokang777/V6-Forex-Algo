...
//| Title=Signals of oscillator 'Average True Range'            |
//| Type=SignalAdvanced                                              |
//| Name=Average True Range                                     |
//| ShortName=ATR                                                    |
//| Class=CSignalATR                                                 |
//| Page=signal_adx                                                  |
//| Parameter=PeriodATR,int,8,Period of calculation                  |
...
class CSignalATR : public CExpertSignal
  {
protected:
   CiATR             m_adx;            // object-oscillator
   //--- adjusted parameters
   int               m_periodATR;      // the "period of calculation" parameter of the oscillator
...
public:
   ...
   void              PeriodATR(int value)              { m_periodATR=value;           }
...
protected:
   ...
   double            ATR(int ind)                      { return(m_adx.Main(ind));     }
   double            DiffATR(int ind)                  { return(ATR(ind)-ATR(ind+1)); }
   int               StateATR(int ind);
   bool              ExtStateATR(int ind);
   bool              CompareMaps(int map,int count,bool minimax=false,int start=0);
  };
//+------------------------------------------------------------------+
//| Constructor                                                      |
//+------------------------------------------------------------------+
CSignalATR::CSignalATR(void) : m_periodATR(14),
...
//--- initial data checks
   if(m_periodATR<=0)
     {
      printf(__FUNCTION__+": period of the ATR oscillator must be greater than 0");
      return(false);
     }
//--- ok
   return(true);
...
//--- initialization of indicators and timeseries of additional filters
   if(!CExpertSignal::InitIndicators(indicators))
      return(false);
//--- create and initialize ATR oscillator
...
//--- initialize object
   if(!m_adx.Create(_Symbol,PERIOD_CURRENT,IND_ATR))
     {
...
//+------------------------------------------------------------------+
int CSignalATR::StateATR(int ind)
  {
   int    res=0;
   double var;
//---
...
//+------------------------------------------------------------------+
bool CSignalATR::ExtStateATR(int ind)
  {
...
//+------------------------------------------------------------------+
bool CSignalATR::CompareMaps(int map,int count,bool minimax,int start)
  {
   ...
   if(DiffATR(idx)>0.0)
     {
      ...
      if(IS_PATTERN_USAGE(1) && DiffATR(idx+1)<0.0 && ATR(idx+1)<30.0)
      ...
      if(IS_PATTERN_USAGE(2) || IS_PATTERN_USAGE(3) || IS_PATTERN_USAGE(4) || IS_PATTERN_USAGE(5))
        {
         ...
         if(IS_PATTERN_USAGE(2) && ATR(idx)>m_extr_osc[1])
         ...
         if(IS_PATTERN_USAGE(5) && CompareMaps(0x62662,5,true) && ATR(idx)>m_extr_osc[1]) // 01100010011001100010b
   ...
   if(DiffATR(idx)<0.0)
     {
      ...
      if(IS_PATTERN_USAGE(1) && DiffATR(idx+1)>0.0 && ATR(idx+1)>70.0)
         result=m_pattern_1;      // signal number 1
      //--- if the model 2, 3, 4 or 5 is used, perform the extended analysis of the oscillator state
      if(IS_PATTERN_USAGE(2) || IS_PATTERN_USAGE(3) || IS_PATTERN_USAGE(4) || IS_PATTERN_USAGE(5))
        {
